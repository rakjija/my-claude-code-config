...
## Change Rules (Code/Docs/IaC)

- 출력 순서 고정: **WHAT → WHY → PATCH**
- **요구사항/입력값이 부족하면** PATCH 전에 질문 1~3개만 먼저
- **승인 대기 금지**, **플랜만 제시 금지** (질문이 필요한 경우만 예외)
- 최소 변경(Blast Radius 최소화)
- 기존 테스트 삭제 금지
...
- 변경 요약 (전/후) + 영향 범위(파일/모듈)

▶ **왜 수정하는가?**

- **Problem**: (현재 이슈 1~2줄)
- **Solution**: (핵심 접근 1~2줄)
- **Effect**: (리스크/부작용 포함 1~2줄)

▶ **PATCH**

- 코드/명령/문서 변경안 (바로 적용 가능한 형태)
```

## Git Rules

- 작업 순서 고정: **Test → Lint/Format → Build → Commit → Push**
- 커밋 메시지 형식: **Conventional Commits**
- **AI Agent 서명/코멘트 커밋 금지** (예: “Generated by …” 제거)
...
- **로그/출력 규칙**
  - 비밀값 로그 출력 금지
  - 에러 메시지에 토큰/비밀번호 포함 금지

## Comment Rules

- 주석은 **Why(이유)** 만 설명
  - 코드로 표현 가능한 내용은 주석 금지

- **필수 주석 대상**
  - 공개 API 함수: 목적 / 파라미터 / 반환값
  - 복잡하거나 직관적이지 않은 로직
...
- 문서는 저장소의 **단일 기준 문서**로 취급 (README.md)
- 세부 규칙 문서: `specs/*`, `workflows/*`, `commands/*`

### Linking

- 내부 문서 링크는 **상대 경로 사용**
  - 예: `specs/security.md`, `workflows/main.md`
- 섹션 참조 시 anchor 사용
  - 예: `workflows/main.md#feature-development`
- 코드 블록 안에 URL 하드코딩 금지

### Content

- 코드와 불일치하는 설명 금지
- 개인 환경 정보(호스트명, 로컬 경로, 실계정) 기재 금지
- 예시는 항상 플레이스홀더 사용
  - 예: `user@example.com`, `/path/to/project`

### Scope

- 문서 수정은 다음 경우만 허용:
  - 코드 변경으로 설명이 깨진 경우
  - README.md가 없거나 섹션이 누락된 경우
- 기능 변경 없는 단순 서술 추가 금지
...
- 자동 배지/아이콘/이미지 추가 금지
- 마케팅 문구, 감정 표현, 강조용 이모지 금지
- “AI가 작성함” 같은 문구 삽입 금지

### README.md 템플릿 (specs/readme.md)
...
### AI Agents 설정 (specs/ai_agents.md)
...
### Internalize (workflows/internalize.md)
...
### commands/setup (commands/setup.md)
